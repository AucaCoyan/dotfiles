...
M.setup = function()
  ...
  local config = {
    -- virtual text is like error lens in VS Code
    virtual_text = true,
    -- show signs
    signs = {
      active = signs,
...
local function lsp_keymaps(bufnr)
  ...
  set_keymap(bufnr, "n", "gh", "<cmd>lua vim.lsp.buf.hover()<CR>", opts)
  ...
  set_keymap(bufnr, "n", "<F8>", '<cmd>lua vim.diagnostic.goto_next({ border = "rounded" })<CR>', opts)
  set_keymap(
    bufnr,
    "n",
    "gl",
...
M.on_attach = function(client, bufnr)
  ...
  lsp_keymaps(bufnr)
  -- highlights the definition on the cursor
